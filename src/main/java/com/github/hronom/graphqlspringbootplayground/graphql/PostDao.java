...
public class PostDao {
    ...
    public List<Post> getRecentPostsAdvanced(int count, int offset, int randomNumberMin, int randomNumberMax) {
        return posts.stream().filter(post -> post.getRandomNumber() >= randomNumberMin && post.getRandomNumber() <= randomNumberMax).skip(offset).limit(count).collect(Collectors.toList());
    }
